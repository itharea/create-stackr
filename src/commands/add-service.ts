...
export async function runAddService(
  ...
  try {
    ...
    //   • { kind: 'write', contents: string }  → write the file in Phase D
    //   • { kind: 'delete' }                   → `rm -f` the file in Phase D
    //     (post-regen no service has tests: true, and a stale file exists)
    //   • { kind: 'noop' }                     → nothing to do
    // Decided here so Phase D is a pure commit sweep with no fresh I/O
    // decisions — matches the A-B-C-D-E invariant.
    const testComposePath = path.join(root, 'docker-compose.test.yml');
    const plannedTestCompose = await planTestComposeRegen(testComposePath, newConfig);
    ...
    if (plannedTestCompose.kind === 'write') {
      try {
        YAML.parse(plannedTestCompose.contents);
      } catch (err) {
        throw new Error(
          `Planned docker-compose.test.yml is not valid YAML: ${(err as Error).message}. ` +
            `Aborting before touching disk.`
        );
      }
    }
    ...
    // 2b. Test compose — write-or-delete per the staged plan.
    if (plannedTestCompose.kind === 'write') {
      await fs.writeFile(testComposePath, plannedTestCompose.contents, 'utf-8');
    } else if (plannedTestCompose.kind === 'delete') {
      await fs.remove(testComposePath);
    }
...
type TestComposePlan =
  | { kind: 'write'; contents: string }
  | { kind: 'delete' }
  | { kind: 'noop' };

/**
 * Decide what to do with `docker-compose.test.yml` for the new config.
 *
 * Unlike the dev/prod composes, this file has no marker blocks — it is
 * regenerated byte-for-byte every time, or deleted if post-regen no
 * service has `tests: true`. The plan is decided here during Phase B so
 * Phase D is a pure commit sweep.
 */
async function planTestComposeRegen(
  testComposePath: string,
  newConfig: StackrConfigFile
): Promise<TestComposePlan> {
  const anyTests = newConfig.services.some((s) => s.backend.tests);
  const exists = await fs.pathExists(testComposePath);

  if (anyTests) {
    return { kind: 'write', contents: renderDockerComposeTest(newConfig) };
  }
  if (exists) {
    return { kind: 'delete' };
  }
